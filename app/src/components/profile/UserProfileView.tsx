...
import { displayRoleLabel, orgRoleLabel } from "@/lib/role-display";
...
export function getProfileDisplayMeta(data: ProfileResponse): {
  displayName: string;
  displaySub: string;
  roleLabel: string;
} {
  const { user, posp, salesTeam } = data;
  const roleLabel = displayRoleLabel(user.role as UserRole, salesTeam?.designation);
  const displayName = salesTeam?.name ?? posp?.name ?? user.email;
  const displaySub = salesTeam
    ? roleLabel
    : posp
      ? `${posp.code}${posp.externalId ? ` · ${posp.externalId}` : ""}`
      : roleLabel;
  return { displayName, displaySub, roleLabel };
}
...
function roleBadgeMeta(appRole: string, orgRole?: string | null) {
  if (orgRole && ORG_ROLE_META[orgRole]) {
    return orgRoleMeta(orgRole);
  }
  return roleMeta(appRole);
}
...
function AccountCard({
  ...
  const rm = roleBadgeMeta(user.role, orgRole);
    ...
    <Card>
      <div style={{ padding: "1.5rem" }}>
        ...
        <dl style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "1rem 1.5rem" }}>
          ...
          <InfoRow label="Role" value={<Badge label={roleLabel} className={rm.cls} />} />
    ...
    <Card>
      <div style={{ padding: "1.5rem" }}>
        ...
        <dl style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "1rem 1.5rem" }}>
          ...
          <InfoRow label="Designation" value={orgRoleLabel(team.designation)} />
    ...
    <Card>
      <div style={{ padding: "1.5rem" }}>
        ...
        <p style={{ fontSize: "0.8rem", color: "#64748b", marginBottom: "1rem" }}>
          People in territory, grouped by org role.
        </p>
      ...
      <div style={{ padding: "1.5rem" }}>
        ...
        {summary.reportingChain.length === 0 ? (
          <div style={{ fontSize: "0.85rem", color: "#94a3b8" }}>
            No reporting chain found for this district.
          </div>
        ) : (
          <>
            <button
...
export function UserProfileView({
  data,
  showHero = true,
}: UserProfileViewProps): React.ReactElement {
  const { user, posp, salesTeam, teamSummary } = data;
  const { displayName, displaySub, roleLabel } = getProfileDisplayMeta(data);
