 ...
 * ~726 districts or ~5.7k cities in bulk. Lists are intersected with the
 * caller's hierarchy scope when restricted.
 */
@ApiTags('Geo')
@Controller('geo')
...
export class GeoController {
  ...
  @ApiOperation({ summary: 'Server-side district typeahead (scoped)' })
  searchDistricts(
    @Query('q') q = '',
  ...
  @Get('cities/search')
  @ApiOperation({ summary: 'Server-side city typeahead (scoped)' })
  searchCities(
    @Query('q') q = '',
    @Query('limit') limit = '20',
  ...
  districtsByIds(
    ...
    return this.geo.districtsByIds(this.parseIds(ids), scope);
  }
  ...
  citiesByIds(
    ...
    return this.geo.citiesByIds(this.parseIds(ids), scope);
  }
